...
7. **`invoicing`** — customer invoice generation and accounts receivable (AR) management (`create_invoice`, `list_invoices`, `record_payment`, `render_invoice`, `ar_aging`).

Each extension directory has its own `package.json` (`{ type: "module", pi: { extensions: ["./index.ts"] } }`) and `tsconfig.json`; `index.ts` is the only file that imports `@earendil-works/pi-coding-agent` and does the pi-facing adaptation. All actual logic lives in plain, pi-agnostic `.ts` modules alongside it (e.g. `categorize.ts`, `reports.ts`, `reconcile.ts`) so it can be unit-tested without a pi harness.
